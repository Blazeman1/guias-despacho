 ...
 * Organiza archivos en carpetas: /{fecha}/{transportista}/{numeroGuia}.pdf
 */
@Slf4j
@Service
public class S3Service {
    ...
    /**
     * Construye la clave S3 organizada por fecha y transportista.
     * Ejemplo: 20240115/transportista_x/GUIA-001.pdf
     */
    public String construirClaveS3(LocalDate fecha, String transportista, String numeroGuia) {
        String fechaStr = fecha.format(DateTimeFormatter.ofPattern("yyyyMMdd"));
        String transportistaNorm = transportista.replaceAll("[^a-zA-Z0-9]", "_").toLowerCase();
        return String.format("%s/%s/%s.pdf", fechaStr, transportistaNorm, numeroGuia);
    }
    ...
    /**
     * Lista objetos en S3 bajo un prefijo específico (por fecha y/o transportista).
     *
     * @param prefijo prefijo para filtrar (ej: "20240115/transportistaX/")
